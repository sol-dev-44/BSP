...
import { ReservationStatus, Reservation, TimeSlot } from '../../types.ts';
...
const ReservationManagement: React.FC = () => {
  ...
  const { data: reservations = [], isLoading, refetch } = reservationsApi.useGetAllReservationsQuery();
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className={`px-3 py-2 rounded-md text-sm font-medium ${filter === 'all' ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}
              onClick={() => setFilter('all')}
            >
              All
            </button>
            <button
              className={`px-3 py-2 rounded-md text-sm font-medium ${filter === 'confirmed' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 hover:bg-green-200'}`}
              onClick={() => setFilter('confirmed')}
            >
              Confirmed
            </button>
            <button
              className={`px-3 py-2 rounded-md text-sm font-medium ${filter === 'pending' ? 'bg-yellow-600 text-white' : 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200'}`}
              onClick={() => setFilter('pending')}
            >
              Pending
            </button>
            <button
              className={`px-3 py-2 rounded-md text-sm font-medium ${filter === 'cancelled' ? 'bg-red-600 text-white' : 'bg-red-100 text-red-800 hover:bg-red-200'}`}
              onClick={() => setFilter('cancelled')}
            >
              Cancelled
            </button>
            <button
              className={`px-3 py-2 rounded-md text-sm font-medium ${filter === 'completed' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-800 hover:bg-blue-200'}`}
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="mt-2 text-base font-medium text-gray-900">No reservations found</p>
              <p className="text-sm text-gray-500">
                {searchTerm ? "Try adjusting your search terms" : "No reservations match the selected filter"}
              </p>
            </div>
          ) : (
                ...
                {paginatedReservations.map((reservation) => (
                  <tr key={reservation.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      {reservation?.riders  || 0 > 0 && <div>{reservation.riders} riders</div>}
                      {(reservation.photo_package || reservation.go_pro_package || reservation.tshirts) && (
                        <div className="text-xs text-gray-500 mt-1">
                          {reservation.photo_package && "Photo pkg, "}
                          {reservation.go_pro_package && "GoPro pkg, "}
                          {reservation?.tshirts || 0 > 0 && `${reservation.tshirts} t-shirts`}
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full 
                        ${reservation.status === 'confirmed' ? 'bg-green-100 text-green-800' : 
                          ...
                          reservation.status === 'refunded' ? 'bg-gray-100 text-gray-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      {(reservation.status === 'confirmed') && (
                        <button 
                          ...
                          onClick={() => handleCompleteInitiate(reservation.id || '')}
                      ...
                      {(reservation.status === 'confirmed' || reservation.status === 'pending') && (
                        <button 
                          ...
                          onClick={() => handleCancelInitiate(reservation.id || '')}
