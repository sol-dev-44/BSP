...
import { getTimeSlotsForDate, getSlotType, getSlotPrice } from '@/config/solarSchedule';
...
export default function BookingForm({ className }: BookingFormProps) {
    ...
    useEffect(() => {
        if (selectedDate) {
            ...
            setTimeout(() => {
                const solarSlots = getTimeSlotsForDate(selectedDate);
    ...
    if (step === 4) {
        return (
            <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-lg mx-auto border-2 border-[#D4605A]">
                <div className="w-20 h-20 bg-[#D4605A]/10 rounded-full flex items-center justify-center mx-auto mb-6">
                    <ShieldCheck className="w-10 h-10 text-[#D4605A]" />
                </div>
                ...
                <button
                    ...
                    className="bg-[#D4605A] hover:bg-[#A84E28] text-white font-bold py-3 px-6 rounded-lg transition-colors uppercase tracking-wider"
                >
                    Book Another
                </button>
            </div>
            ...
            <div className="flex justify-center items-center space-x-4 mb-8">
                {[1, 2, 3].map((s) => (
                    <div key={s} className="flex items-center">
                        <div className={`flex flex-col items-center gap-2 ${step >= s ? 'text-[#D4605A]' : 'text-stone-300'}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${step >= s ? 'bg-[#D4605A] text-white' : 'bg-stone-200'}`}>
                                {s}
                            </div>
                        ...
                        {s < 3 && <div className={`w-12 h-0.5 mx-2 ${step > s ? 'bg-[#D4605A]' : 'bg-stone-200'}`} />}
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            >
                                <div className="bg-white rounded-2xl shadow-xl p-6 border border-stone-200">
                                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2 text-foreground font-serif">
                                        <Calendar className="text-[#D4605A]" /> Select Date
                                    </h3>
                                    <DateSelector
                                ...
                                <div className="flex justify-end">
                                    <button
                                        ...
                                        className="bg-[#D4605A] hover:bg-[#A84E28] text-white font-bold px-8 py-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#D4605A]/20 uppercase tracking-wider"
                                    >
                                        Continue
                                    </button>
                                </div>
                            ...
                            >
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <button
                                        ...
                                        className="bg-[#D4605A] hover:bg-[#A84E28] text-white font-bold px-8 py-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#D4605A]/20 flex items-center gap-2 uppercase tracking-wider"
                                    >
                ...
                <div className="lg:col-span-4">
                    <div className="sticky top-24 space-y-4">
                        ...
                        />
                        <div className="bg-[#D4605A]/10 p-4 rounded-xl border border-[#D4605A]/20 flex items-start gap-3">
                            <ShieldCheck className="w-5 h-5 text-[#D4605A] mt-0.5 shrink-0" />
                            <div>
