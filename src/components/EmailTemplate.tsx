...
export const EmailTemplate: React.FC<Readonly<EmailTemplateProps>> = ({
    ...
    return (
        <div style={{ fontFamily: 'Georgia, serif', lineHeight: '1.6', color: '#3D2B1F', maxWidth: 600, margin: '0 auto' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #00f0ff, #7b2dff)', padding: 24, textAlign: 'center', borderRadius: '12px 12px 0 0' }}>
                <h1 style={{ color: '#0d0d1f', margin: '0 0 4px', fontFamily: 'Georgia, serif' }}>Booking Confirmed!</h1>
                <p style={{ color: '#ff00ff', margin: 0, fontSize: 14, fontWeight: 'bold' }}>Parasail Flight</p>
            </div>

            {/* Retro stripe bar */}
            <div style={{ display: 'flex', height: 6 }}>
                <div style={{ flex: 1, backgroundColor: '#00f0ff' }} />
                <div style={{ flex: 1, backgroundColor: '#ff00ff' }} />
                <div style={{ flex: 1, backgroundColor: '#7b2dff' }} />
            </div>

            <div style={{ padding: 24, backgroundColor: '#0d0d1f' }}>
                ...
                <div style={{ backgroundColor: '#fff', padding: 20, borderRadius: 8, margin: '16px 0', border: '1px solid #ff00ff' }}>
                    <h3 style={{ marginTop: 0, color: '#7b2dff' }}>Trip Details</h3>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <tbody>
                            <tr>
                                <td style={{ padding: '6px 0', color: '#7b2dff' }}>Date</td>
                                <td style={{ padding: '6px 0', fontWeight: 'bold', textAlign: 'right', color: '#3D2B1F' }}>{displayDate}</td>
                            </tr>
                            <tr>
                                <td style={{ padding: '6px 0', color: '#7b2dff' }}>Time</td>
                                <td style={{ padding: '6px 0', fontWeight: 'bold', textAlign: 'right', color: '#3D2B1F' }}>{displayTime} (Mountain)</td>
                            </tr>
                            <tr>
                                <td style={{ padding: '6px 0', color: '#7b2dff' }}>Group Size</td>
                ...
                <div style={{ backgroundColor: '#fff', padding: 20, borderRadius: 8, margin: '16px 0', border: '1px solid #ff00ff' }}>
                    <h3 style={{ marginTop: 0, color: '#7b2dff' }}>Itemized Receipt</h3>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <tbody>
                            {(addOns?.photo_package || 0) > 0 && (
                                <tr>
                                    <td style={{ padding: '6px 0', color: '#7b2dff' }}>HD Photo Package x {addOns!.photo_package}</td>
                                    <td style={{ padding: '6px 0', textAlign: 'right', fontWeight: 600, color: '#3D2B1F' }}>${photoTotal.toFixed(2)}</td>
                                </tr>
                            )}
                            {(addOns?.gopro_package || 0) > 0 && (
                                <tr>
                                    <td style={{ padding: '6px 0', color: '#7b2dff' }}>GoPro Rental x {addOns!.gopro_package}</td>
                            ...
                            <tr style={{ borderTop: '2px solid #ff00ff' }}>
                                <td style={{ padding: '12px 0 0', fontWeight: 'bold', fontSize: 16, color: '#3D2B1F' }}>Total Paid</td>
                                <td style={{ padding: '12px 0 0', textAlign: 'right', fontWeight: 'bold', fontSize: 18, color: '#00f0ff' }}>${totalAmount.toFixed(2)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div style={{ margin: '16px 0' }}>
                    <h3 style={{ color: '#7b2dff' }}>Location</h3>
                    <p style={{ margin: '4px 0', color: '#3D2B1F' }}>
                        <strong>Flathead Harbor Marina</strong><br />
                        Lakeside, MT
                    </p>
                    <p style={{ fontSize: 13, color: '#00f0ff', fontWeight: 600 }}>Please arrive 15 minutes before departure.</p>
                </div>

                <p style={{ fontSize: 13, color: '#7b2dff', fontStyle: 'italic' }}>
                    This email confirmation is NOT required to board the boat. We have your name on the manifest.
                </p>

                <p style={{ color: '#3D2B1F' }}>
                    Questions? Contact us at <a href="mailto:bigskyparasailing@gmail.com" style={{ color: '#00f0ff' }}>bigskyparasailing@gmail.com</a> or call <a href="tel:4062706256" style={{ color: '#00f0ff' }}>(406) 270-6256</a>.
                </p>

                <p style={{ color: '#3D2B1F' }}>See you on the water!</p>
                <p style={{ fontSize: 12, color: '#7b2dff' }}>Big Sky Parasail LLC</p>
            </div>
